...
def generate_conditional_msa_sequences(directory_path, output_csv_file, max_retries=5, to_device='cuda', total_sequences=5):
    """
    Generates AVP sequences using EvoDiff Conditional Generation with MSA_OA_DM_MAXSUB and saves the results to a CSV file.
    ...
    # Create empty DataFrame with the desired columns
    ...
    # If file exists, read the current ID from the last row
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".a3m"):
            ...
            # Run the model total_sequences times with random seq_length each time
            for _ in range(total_sequences):
                retries = 0
                while retries < max_retries:
                    try:
                        seq_length = random.randint(15, 25)
                        ...
                        # Create a new row and append to CSV immediately
                        ...
                        # Append to CSV file
                        new_row.to_csv(output_csv_file, mode='a', header=False, index=False)
